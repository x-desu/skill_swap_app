...
const PRIMARY = '#ff1a5c'; // Rose Neon
...
const PRIMARY_LIGHT = 'rgba(255, 26, 92, 0.15)';
...
export default function PaywallScreen() {
  ...
  console.log('Razorpay Key ID:', extra.razorpayKeyId);
  ...
  const [selectedIdx, setSelectedIdx] = useState(1); // Default to Value Pack
  ...
  const onPurchase = async () => {
    ...
    try {
      ...
      if (success) {
        if (user?.uid) {
          await upsertUserProfile(user.uid, { hasSeenPaywall: true });
        }
...
const styles = StyleSheet.create({
  ...
  restore: { fontSize: 16, color: PRIMARY, fontWeight: '700', textShadowColor: 'rgba(255, 26, 92, 0.4)', textShadowRadius: 10 },
  sheet: {
    flex: 1,
  ...
  approxPriceSelected: { color: DARK_TEXT_MUTED },
  ...
  warn: { textAlign: 'center', color: PRIMARY, marginVertical: 16, fontWeight: '600' },
  skipBtn: {
    marginTop: 16,
    alignItems: 'center',
    paddingVertical: 10,
